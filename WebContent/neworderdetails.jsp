   ...
   <strong><b style="font-family: serif;margin-top: 10px;font-size: 11px;"><img src="images/Delete_Icon.png" height="15x" width="20px"></img><a href="deletesearchproduct.jsp" style="padding: 8px;background-color: azure">Delete order</a></b></strong> -->
     ...
     <a href="getAddress.jsp?lat=<%=lat%>&lng=<%=lng%>" style=" color: blue"> Get Address</a>
				...
				<%
					...
					ps1 = con.prepareStatement("SELECT  * FROM merchantbranch where id=?");
									...
									while (rs1.next()) {
										...
										merchant_mob1 = rs1.getString("mobile_number");
					...
					<td><%=rs1.getString("mobile_number")%><br><%=rs1.getString("alt_mobile_number_one")%><br><%=rs1.getString("alt_mobile_number_two")%></td>
					...
					<%
						ps4 = con
										...
										while (rs4.next()) {
											merchant_name = rs4.getString(8)+" "+branch_name;
					%>
				...
				<%
					...
					<td><a href="deliverystatus.jsp?orderid=<%=orderid%>" style="color: darkblue">status</a><br><br>
					<a href="deliverycancel.jsp?orderid=<%=orderid%>" style="color: darkblue">cancel</a></td>
					<td>	
						
					<form action="deliver.jsp" method="post">
						 <select style="width: 77px;padding: 5px 3px 2px 2px;" name="deliveryagentname">
