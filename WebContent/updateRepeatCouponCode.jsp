...
<body >
				<%
					...
					ResultSet rs = null;
				     ...
				     try
	              {
	      		ps3 = con.prepareStatement("update  repeat_discount set order_number=?,type=?,value=?,custome_message=?,maximum_discount_value=?,manimum_order_value=? where  order_number=?");
				
				ps3.setInt(1,orderNumber);
				ps3.setInt(2,discountType);
				ps3.setInt(3,discountValue);
				ps3.setString(4,discountMessage);
				ps3.setDouble(5,maximumDiscountAmount);
				ps3.setDouble(6,minimumOrderAmount);
				ps3.setInt(7,orderNumber);
				ps3.executeUpdate();
				con.close();
				%>
					<script>
       alert(' Coupon Code Successfully updated');
         window.location='repeat_order_discount.jsp';
				...
				%>
					...
					<script>
       alert('orderNumber can not update');
         window.location='repeat_order_discount.jsp';
</script>
